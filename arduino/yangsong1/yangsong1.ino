...
float EMF_AVR[11] = {
    0,
};                             //평균값을 저장하기 위한 공간, averaging_count가 10이기 때문에 11로 설정함, averaging_count가 커지면 더 큰 값으로 변경해야 함
...
void loop()
{
  ...
  { //every 1 second, if you change set_time to 60, every 1 minute
